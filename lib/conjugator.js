...
function getInfinitiveEnding(infinitive) {
  var idx = getInfinitiveStem(infinitive).length;
  var verb = getVerb(infinitive);
  if (verb.seperable) {
    idx += verb.seperable.prefix.length;
  }
  return infinitive.substring(idx);
}
...
function hasInseperablePrefix(infinitive) {
  ...
  return inseperableRe.test(infinitive) && 
...
function getPartizip(infinitive, explain) {
  ...
  if (verb.partizip) {
    ...
    var result = combineStemAndEnding(infinitive, 'partizip', null, stem, ending, explain);
