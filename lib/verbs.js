...
function load(filename){
  ...
  return normalizeIrregularity(Object.keys(data).map(function(infinitive){
    ...
    if (idx > -1) {
      ...
      var copyInfinitive = infinitive.substring(idx + 1);
      var copyVerb = data[copyInfinitive];
      if (!copyVerb) {
        throw new Error('verb for seperable copy is not defined [' + copyInfinitive + ']');
      }
      Object.keys(copyVerb).forEach(function(k){
        if (!verb.hasOwnProperty(k)) {
          verb[k] = copyVerb[k];
        }
      });
    }
